...
def build_steps() -> list[Step]:
    return [
        ...
        Step(
            ...
            command=pytest_cmd("pico_vllm/tests/test_environment.py"),
            requirement=always,
        ),
        Step(
            layer="2-ops",
            name="CPU torch operators",
            command=pytest_cmd("pico_vllm/tests/test_torch_ops.py"),
            requirement=always,
        ),
        Step(
            layer="2-ops",
            name="CUDA paged prefill attention",
            command=pytest_cmd("pico_vllm/tests/test_prefill_paged_attn.py"),
            requirement=requires_cuda,
        ),
        Step(
            layer="3-single-card",
            name="single CUDA card tiny model",
            command=pytest_cmd("pico_vllm/tests/test_single_card_smoke.py"),
            requirement=requires_cuda,
        ),
        Step(
            layer="4-single-node-multi-card",
            name="single-node NCCL all-reduce smoke",
            command=torchrun_cmd(
                2,
                "pico_vllm/tests/distributed/test_single_node_multi_gpu_smoke.py",
            ),
        ...
        Step(
            ...
            command=torchrun_cmd(2, "pico_vllm/tests/distributed/test_tp_tiny_smoke.py"),
            requirement=requires_two_gpus,
        ),
    ]
